...
class SessionService:
    async def list_sessions(self, user_id: str, app_name: str = APP_NAME, db_path: str = DB_PATH):
        ...
        return [{"id": row[0], "state": row[1]} for row in rows]

    async def create_session(self, user_id: str, app_name: str = APP_NAME, state: str = initial_state, db_path: str = DB_PATH):
        with get_connection(db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO sessions (app_name, user_id, state) VALUES (?, ?, ?)",
    ...
    async def update_session_state(self, session_id: int, new_state: dict, db_path: str = DB_PATH):
        with get_connection(db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    async def get_session_state(self, session_id: int, db_path: str = DB_PATH):
        with get_connection(db_path) as conn:
            cursor = conn.cursor()
...
async def get_or_create_session(user_id: str):
    ...
    new_session = await session_service.create_session(user_id)
